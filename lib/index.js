...
class ThirdPersonCameraControls {
  ...
  update(rayResult) {
    ...
    this.spherical.radius = Math.max(this.minDistance, Math.min(this.maxDistance, this.spherical.radius)); // copy maths to actual three.js camere
...
const ThirdPersonCharacterControls = _ref => {
  ...
  const {
    actions,
    mixer
  } = useThirdPersonAnimations(characterObj, animationPaths);
  const {
    animation,
    isMoving
